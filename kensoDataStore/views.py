		...
		<html>
			...
			<script>$("#input").keypress(function (e) {
    					if (e.keyCode == 13) {
      						...
      						document.getElementById("short-display").innerHTML = "";
							document.getElementById("neutral-display").innerHTML = "";
							document.getElementById("long-display").innerHTML = "";
      						$.get("/api/getVolatility?symbols=" + symbols, function( inp ) {
  								...
  								for (var property in data.neutral){
  									if(data.neutral.hasOwnProperty(property)){
  										$("#neutral-display").append(property + "<br>");
  									}
  								}
  								for (var property in data.long){
  									if(data.long.hasOwnProperty(property)){
  										$("#long-display").append(property + "<br>");
  									}
  								}
							});
    					}
